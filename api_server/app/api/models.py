...
class ErrorResponse(BaseModel):
    ...
    code: str | None = Field(
        None, description="Código de error para identificación programática", example="INVALID_INPUT"
    )
...
class DataSource(BaseModel):
    ...
    name: str = Field(..., description="Nombre de la fuente de datos", example="MarketDatabase")

    source_type: str = Field(..., description="Tipo de fuente (API, database, web, etc.)", example="API")

    reliability: float = Field(
        ..., description="Puntuación de confiabilidad de la fuente (0.0-1.0)", ge=0.0, le=1.0, example=0.85
    )

    timestamp: float = Field(
        default_factory=time.time, description="Marca de tiempo cuando se consultó la fuente", example=1633036800.1234
    )
...
class QueryRequest(BaseModel):
    ...
    agent_preference: str | None = Field(
        None, description="Preferencia de agente específico para procesar la consulta", example="analysis_agent"
    )
...
class QueryResponse(BaseModel):
    ...
    agent: str = Field(..., description="Identificador del agente que procesó la consulta", example="analysis_agent")

    processing_time: float = Field(..., description="Tiempo de procesamiento en segundos", example=1.45)

    confidence: float = Field(
        ..., description="Nivel de confianza de la respuesta (0.0-1.0)", ge=0.0, le=1.0, example=0.87
    )
    ...
    data_sources: list[DataSource] | None = Field(
        ...
        example=[
            ...
            {"name": "API MarketTrends", "source_type": "API", "reliability": 0.85, "timestamp": 1633036820.5678},
        ],
    )
...
class DataLookupRequest(BaseModel):
    ...
    lookup_type: str = Field(
        ..., description="Tipo de búsqueda a realizar (market, news, industry, web, company)", example="market"
    )

    query: dict[str, Any] = Field(
        ...,
        description="Parámetros específicos para la búsqueda",
        example={"term": "ecommerce trends spain 2023", "limit": 5, "filter": "last_year"},
    )
...
class DataLookupResponse(BaseModel):
    ...
    success: bool = Field(..., description="Indica si la búsqueda fue exitosa", example=True)
    ...
    query: dict[str, Any] = Field(
        ...
        example={"term": "ecommerce trends spain 2023", "limit": 5},
    )

    lookup_type: str = Field(..., description="Tipo de búsqueda realizada", example="market")

    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadatos adicionales de la búsqueda",
        example={"data_freshness": "1 día", "coverage": "Spain, Portugal", "sources_used": 3},
    )
